...
class PocketBase {
  ...
  ///
  /// Enhanced filter operators are supported:
  /// - `contains(field, value)` - check if field contains value
  /// - `containsAny(field, value)` - check if field contains any of the values
  /// - `containsAll(field, value)` - check if field contains all of the values
  /// - `notContains(field, value)` - check if field does not contain value
  /// - `startsWith(field, value)` - check if field starts with value
  /// - `endsWith(field, value)` - check if field ends with value
  /// - `regex(field, pattern)` - check if field matches regex pattern
  /// - `lengthGt(field, length)` - check if field length is greater than length
  /// - `lengthLt(field, length)` - check if field length is less than length
  ///
  /// Example:
  ///
  /// ```dart
  ...
  ///   "title ~ {:title} && created >=
  /// {:created} && contains(description, 'dart')",
  ...
  Future<T> send<T extends dynamic>(
    ...
    // note: the optional generic type is to ensure that the expected
    // response data type is returned, the extends is to explicitly
    // infer by default to dynamic
    String path, {
    String method = "GET",

...
class RecordService extends BaseCrudService<RecordModel> {
  ...
  Future<void> delete(
    ...
    return super.delete(id, body: body, query: query, headers: headers).then((
      _,
    ) {
      if (client.authStore.record != null &&
  ...
  Future<RecordAuth> authWithOAuth2(
    ...
    try {
      provider = authMethods.oauth2.providers.firstWhere(
  ...
  ///
  /// This method searches across all searchable text fields in the collection.
  /// The [query] parameter is the search term to look for.
  /// The [limit] parameter limits
  /// the number of results (default: 50, max: 1000).
  /// The [offset] parameter specifies the number of records to skip.
  Future<ResultList<RecordModel>> search(
    String query, {
    int limit = 50,
    int offset = 0,
    String? expand,
    ...
    final enrichedQuery = Map<String, dynamic>.of(queryParams);
    enrichedQuery["q"] = query;
    enrichedQuery["limit"] ??= limit;
    enrichedQuery["offset"] ??= offset;
    enrichedQuery["expand"] ??= expand;
    enrichedQuery["fields"] ??= fields;

    return client
        .send<Map<String, dynamic>>(
          "$baseCollectionPath/search",
          query: enrichedQuery,
          headers: headers,
        )
        .then(
          (data) => ResultList<RecordModel>.fromJson(data, itemFactoryFunc),
        );
  }
}
